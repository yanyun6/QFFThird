 ...
 * RandomAccessFile : (1） seek(int index);可以将指针移动到某个位置开始读写;
​                    （2）  setLength(long len);给写入文件预留空间：
 */
public class MultiTheradDownLoad {
    ...
    //计数器，可以记录程序的字节码执行位置，每一个线程在工作的时候都有一个独立的计数器
    ...
    //有参构造函数，先构造需要的数据   文件地址          线程数
    public MultiTheradDownLoad(String filepath, int threadNum) {
        this.filepath = filepath;
        this.threadNum = threadNum;
        startPos = new long[this.threadNum];//保留每个线程数据起始位置的数组大小应为线程数
...
//        File file = null;
        File tmpfile = null;
        HttpURLConnection httpcon = null;//可以来获取连接 ，设置http传输参数等
        ...
        //substring 方法来截取该字符串的子字符串 ;int lastIndexof 方法:返回指定字符在此字符串中最后一次出现处的索引。
        // 三元运算符的使用
        tmpfilename = filename + "_tmp";
        try {
            //HttpURLConnection(抽象类)只能通过URL.openConnection()方法创建具体的实例
            ...
            file = new File("F:" + File.separator + "云盘存储目录", filename);
            if (!file.exists()) {
                file.createNewFile();
            }
            tmpfile = new File("F:" + File.separator + "云盘存储目录" + File.separator + tmpfilename);
            ...
            if (file.exists() && file.length() == fileLength) {
                ...
                setBreakPoint(startPos, endPos, tmpfile);//调用类内部类
                ExecutorService exec = Executors.newCachedThreadPool();//开启线程池线程池
                for (int i = 0; i < threadNum; i++) {
                    exec.execute(new DownLoadThread(startPos[i], endPos[i],
                            this, i, tmpfile, latch));
                }
                latch.await();//当你的计数器减为0之前，会在此处一直阻塞。主线程在阻塞
                exec.shutdown();
            }
        ...
        if (file.length() == fileLength) {//这说明已经下载完成，删除临时文件
            if (tmpfile.exists()) {
    ...
    private void setBreakPoint(long[] startPos, long[] endPos, File tmpfile) {
        //  (1） seek(int index);可以将指针移动到某个位置开始读写;
        // （2）  setLength(long len);给写入文件预留空间：
        RandomAccessFile rantmpfile = null;
        try {
            if (tmpfile.exists()) {
                System.out.println("the download has continued!!");
                rantmpfile = new RandomAccessFile(tmpfile, "rw");//rw读写
                for (int i = 0; i < threadNum; i++) {
    ...
    class DownLoadThread implements Runnable {
        ...
        private CountDownLatch latch = null;//计数器
        ...
        public DownLoadThread(long startPos, long endPos,
            ...
            try {
                this.downloadfile = new RandomAccessFile(this.task.file,
                        "rw");
...
//                            从HTTP响应消息获取状态代码。
                        if (httpcon.getResponseCode() != HttpURLConnection.HTTP_OK //HTTP状态码200：确定。
                                && httpcon.getResponseCode() != HttpURLConnection.HTTP_PARTIAL) {//HTTP状态码206：部分内容。
                            this.task.bool = true;
